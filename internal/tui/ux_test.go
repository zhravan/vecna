...
func TestTabSwitchDeltaFromKey(t *testing.T) {
	...
	for key, want := range tests {
		...
		if want == 0 { if ok { t.Fatalf("%q: expected no match", key) }; continue }
		if !ok || got != want { t.Fatalf("%q: got (%d,%v), want (%d,true)", key, got, ok, want) }
	}
}
...
func TestNormalizeDroppedPaths(t *testing.T) {
	...
	if err := os.WriteFile(first, []byte("one"), 0600); err != nil { t.Fatal(err) }
	if err := os.WriteFile(second, []byte("two"), 0600); err != nil { t.Fatal(err) }
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(got) != tt.want { t.Fatalf("got %v, want %d paths", got, tt.want) }
		})
	}
}