...
// StartLocalForward listens locally and proxies connections through an SSH client.
...
// StartRemoteForward exposes a remote listener and forwards accepted sessions to a local address.
func StartRemoteForward(ctx context.Context, client *ssh.Client, spec ForwardSpec) (*ForwardHandle, error) {
	...
	go func() {
		...
		for {
			conn, err := ln.Accept()
			if err != nil {
				if ctx.Err() != nil { done <- nil } else { done <- err }
				return
			}
...
// StartDynamicForward exposes a local SOCKS5 listener over the SSH connection.
...
func proxyConn(ctx context.Context, src net.Conn, dial func() (net.Conn, error)) {
	...
	select { case <-ctx.Done(): _ = src.Close(); _ = dst.Close(); case <-waitGroupDone(&wg): }
}
...
func handleSOCKS5(ctx context.Context, client *ssh.Client, conn net.Conn) {
	...
	dst, err := client.Dial("tcp", net.JoinHostPort(host, port)); if err != nil { writeSOCKSError(conn, 5); return }
	defer dst.Close()
...
func writeSOCKSError(conn net.Conn, code byte) { _, _ = conn.Write([]byte{5, code, 0, 1, 0, 0, 0, 0, 0, 0}) }

func deadline(ctx context.Context) (t time.Time) {
	if d, ok := ctx.Deadline(); ok { return d }
