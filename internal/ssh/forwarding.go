...
func handleSOCKS5(ctx context.Context, client *ssh.Client, conn net.Conn) {
	...
	header := make([]byte, 4)
	if _, err = io.ReadFull(br, header); err != nil || header[0] != 5 || header[1] != 1 {
		return
	}
	atyp := header[3]
	var host string
	...
	dst, err := client.Dial("tcp", net.JoinHostPort(host, port))
	if err != nil {
	...
	case <-ctx.Done():
		_ = conn.Close()
		_ = dst.Close()
	case <-func() <-chan struct{} { ch := make(chan struct{}); go func() { wg.Wait(); close(ch) }(); return ch }():
	}
}
